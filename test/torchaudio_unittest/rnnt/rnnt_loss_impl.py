...
class RNNTLossTest:
    ...
    def test_costs_and_gradients_B1_T2_U3_D5_fp32(self, reuse_logits_for_grads):
    ...
    def test_costs_and_gradients_B1_T2_U3_D5_fp16(self, reuse_logits_for_grads):
        data, ref_costs, ref_gradients = get_numpy_data_B1_T2_U3_D5(
            dtype=np.float16
        )
        data = numpy_to_torch(data=data, device=self.device, requires_grad=True)
    ...
    def test_costs_and_gradients_B2_T4_U3_D3_fp32(self, reuse_logits_for_grads):
    ...
    def test_costs_and_gradients_B2_T4_U3_D3_fp16(self, reuse_logits_for_grads):
        data, ref_costs, ref_gradients = get_numpy_data_B2_T4_U3_D3(
            dtype=np.float16
        )
        data = numpy_to_torch(data=data, device=self.device, requires_grad=True)
    ...
    def test_costs_and_gradients_random_data_with_numpy_fp32(self, reuse_logits_for_grads):
        seed = 777
        for i in range(5):
    ...
    def test_rnnt_nonfused_log_softmax(self):
        for random in [False, True]:
            ...
            data["reuse_logits_for_grads"] = False
